...
BOOL _pl2303_io_stop( struct ExecIFace *IExec,struct PL2303DeviceBase *devBase,struct IOStdReq *io ) {

	#if DebugLevel > 0
		IExec->DebugPrintF( "[%s] (%s@%ld) Command = CMD_STOP ) Don't know how to handle\n",DEVNAME,__func__,__LINE__);
	#endif
